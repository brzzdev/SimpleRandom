...
The generator is `scripts/generate-licences.swift`, run by `just licences` whenever a dependency is added, removed or re-pinned. It reads each package's licence file out of `.build/checkouts` and classifies it by its text rather than its file name, because the file is called `LICENSE`, `LICENCE` or `LICENSE.txt` depending on the author and none of those spellings says what is in it. Every recogniser runs and exactly one must match, so a dual-licensed body is refused rather than read as whichever recogniser was written first.

**It fails rather than guesses**, and every guess it could make is a failure instead: a missing checkout, no licence file and no stated terms, terms it cannot classify, terms it can classify two ways, a package that has both a licence file and a stated entry, or a body carrying a sequence the generated raw literal cannot hold verbatim. Shipping a package as `Unknown` — or quietly dropping it, or crediting one licence for another — is the failure this screen exists to prevent, and it would be invisible in a diff of an already-long generated file. What the MIT recogniser promises is narrower than "this is MIT": it is "this carries MIT's three hallmarks and no other licence's", which a derivative adding a clause would still satisfy. That residual is named in the script rather than papered over with a blocklist of known derivatives, which could never be complete.

TCA26 is the one package with no licence file, and the terms its README states (`© 2026 Point-Free, Inc. All rights reserved.`) are written into the generator by hand.

**Nothing gates a build on the file being current**, and nothing should — forgetting to regenerate after a re-pin is the expected failure, not one to punish a build for. `.github/workflows/licences.yml` runs the generator weekly and opens a PR with whatever moved, so forgetting is harmless; no-op weeks open no PR. It mirrors SimpleTrackerForAniList's job, including its GitHub App token, which this repo does not yet need — it has no ruleset requiring signed commits and no CI for a PR to trigger.

**Delete All Lists** is disabled when there is nothing for it to take. It raises a `confirmationDialog` stating the real counts and the real blast radius — "Delete all 7 lists, 42 items and 3 combos?" / "This removes them from this iPhone and from iCloud on all your devices. It cannot be undone." The specificity is the safety mechanism; a typed `DELETE` confirmation was rejected as a keyboard on a screen that otherwise needs none. There is no local-only variant — deletes are hard and sync-propagating, and stopping the engine to wipe locally only means CloudKit re-seeds the device on restart. It earns its place because uninstalling leaves the CloudKit private database intact.

**The dialog drops a clause rather than counting it at zero.** "Delete all 3 lists, 10 items and 0 combos?" is what everyone who has never opened the Combine tab would otherwise read, and grammar agreement does not suppress a zero — it only agrees the noun with its number. So the title is one of five whole phrases, chosen by which counts are non-zero: all three, lists and items, lists and Combos, lists alone, or Combos alone. Items imply Lists, so those five are all that exist, and the all-zero case cannot arise because the row is disabled there. Five entries rather than clauses joined in Swift, for the reason under **Composed strings**: a join is the one construction that cannot be translated.

**Combos go with the Lists**, which is the one place the app deletes a Combo you did not aim at. This is the gesture that gets rid of everything, and Combos surviving it would leave the Combine tab holding rows that can no longer name what they were arrangements of — an end state nobody asked for by tapping a button whose dialog said everything was going. It is what separates this from deleting Lists one at a time, where a Combo stays and silently shrinks (ADR-0023 draws the same line from the other side). The dialog therefore counts Combos too: a blast radius that under-states itself is the failure this whole mechanism exists to prevent. Because Combos are in scope, "disabled when there are no Lists" becomes disabled when there are neither — a Combo can outlive every List it was built from, and a button that refused to clear it would be dimmed with something still to delete.
