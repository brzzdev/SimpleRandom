...
The generator is `tools/generate-licenses.swift`, run by `just licenses` whenever a dependency is added, removed or re-pinned. It reads each package's licence file out of `.build/checkouts` and classifies it by its text rather than its file name, because the file is called `LICENSE`, `LICENCE` or `LICENSE.txt` depending on the author and none of those spellings says what is in it. **It fails rather than guesses**: a package with no licence file and no stated terms, or with terms it cannot classify, stops the run and names itself. Shipping a package as `Unknown` — or quietly dropping it — is the failure this screen exists to prevent, and it would be invisible in a diff of an already-long generated file. TCA26 is the one package with no licence file, and the terms its README states (`© 2026 Point-Free, Inc. All rights reserved.`) are written into the generator by hand.
