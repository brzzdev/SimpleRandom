...
**The pick is a `mutating func` on `RandomiseFeature.State`, called from the reducer and from `State.init`.** No client sits between the feature and the generator, which is what **Seams**' randomness rule is protecting — but the logic is not literally *in* the reducer, because the opening result is drawn as the state is built rather than on an action. That is what leaves `drawToken` already moved before the sheet's view exists, and so what keeps the haptic and the announcement silent at presentation. The cost is that the pick reads `@Dependency(\.withRandomNumberGenerator)` at construction, so a `State` built outside a reducer's dependency scope — a preview, or a test that forgets — falls through to the live generator rather than the one it meant to use.
